    ...
    def draw(self, img):
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1:
                ...
                cv2.putText(img,"{}_a:({}, {})".format(Pose.kpt_names[kpt_a_id], x_a, y_a), (x_a,y_a), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255))
            kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
            global_kpt_b_id = self.keypoints[kpt_b_id, 0]
            if global_kpt_b_id != -1:
                x_b, y_b = self.keypoints[kpt_b_id]
                cv2.circle(img, (int(x_b), int(y_b)), 3, Pose.color, -1)
                #cv2.putText(img,"{}_b:({}, {})".format(Pose.kpt_names[kpt_b_id], x_b, y_b), (x_b,y_b), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0))
